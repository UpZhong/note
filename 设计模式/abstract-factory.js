 ...
 * 但是抽象工厂着重的就是为一个产品簇选择实现，定义在抽象工厂里面的方法通常是有联系的，它们都是产品的某一部分或者是相互依赖的。如果抽象工厂里面只定义一个方法，直接创建产品，那么就退化成为工厂方法。
 *
 * 何时使用？
...
// 客户端
var client = new ConcretFactory1();
var ca = client.createProductA();
var cb = client.createProductB();
ca.display();
cb.close();
